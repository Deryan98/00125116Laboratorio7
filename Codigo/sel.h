...
Matrix createLocalK(int e,mesh &m){
    float T,Kap,L,Delta,Eta,Ae,J,D;
    ...
    T = m.getParameter(VAR_TAO);
    productRealMatrix(T*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    Kap = m.getParameter(VAR_KAPPA);
    productRealMatrix(Kap*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixB);
    Delta = m.getParameter(VAR_DELTA);
    productRealMatrix((-1)*(Delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixF);
    L = m.getParameter(VAR_LAMBDA);
    productRealMatrix((J*L)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixE);

    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixD);

    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixB,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixE,matrixF,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(VAR_PSIX);
    float f_y = m.getParameter(VAR_PSIY);
    float Eta = m.getParameter(VAR_ETA);
